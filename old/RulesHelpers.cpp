...
// [[Rcpp::export]]
NumericVector minDiscPath(Graph *pag, int a,int b,int c,bool verbose=false){
  int p = pag->size();
  ...
  if (d_vals.length()>0){
    ...
    while (counter < list_length){
      ...
      m = mpath.length();
      d = mpath(m-1);
      if (pag->getAmatVal(c,d)==0 && pag->getAmatVal(d,c)==0){
      ...
      if (pag->getAmatVal(d,c)==2 && pag->getAmatVal(c,d)==3 && pag->getAmatVal(pred,d)==2){
        visited(d) = true;
...
NumericVector minUncovPdPath(int p,Graph *G, int alpha,int beta,int gamma,bool verbose){
  ...
  bool cond1 = G->getAmatVal(beta,gamma)==1 || G->getAmatVal(beta,gamma)==2;
  bool cond2 = G->getAmatVal(gamma,beta)==1 || G->getAmatVal(gamma,beta)==3;
  bool cond3 = G->getAmatVal(gamma,alpha)==0 && G->getAmatVal(alpha,gamma)==0;
  ...
  if (!done){
    ...
    for (int theta=0;theta<p;++theta){
      cond1 = G->getAmatVal(beta,theta)==1 || G->getAmatVal(beta,theta)==2;
      cond2 = G->getAmatVal(theta,beta)==1 || G->getAmatVal(theta,beta)==3;
      cond3 = G->getAmatVal(theta,alpha) == 0 && G->getAmatVal(alpha,theta)== 0 && !visited[theta];
      if (cond1 && cond2 && cond3){
    ...
    if (theta_vals.length()>0){
      ...
      while ((counter<path_list.length()) && (!done)){
        ...
        d = mpath(m-1);
        visited[d]=true;
        cond1 = G->getAmatVal(d,gamma)==1 || G->getAmatVal(d,gamma)==2;
        cond2 = G->getAmatVal(gamma,d)==1 || G->getAmatVal(gamma,d)==3;
        if (cond1 && cond2){
          ...
          uncov=true;
          for (int i=0;i<n-2;++i){
            cond1 = (G->getAmatVal(path(i),path(i+2))==0) && (G->getAmatVal(path(i+2),path(i))==0);
            if (!cond1){
              uncov = false;
              break;
            }
          ...
          for (int i=0;i<p;++i){
            cond1 = G->getAmatVal(d,i) == 1 || G->getAmatVal(d,i)==2;
            cond2 = G->getAmatVal(i,d) == 1 || G->getAmatVal(i,d)==3;
            cond3 = !visited[i];
            if (cond1 && cond2 && cond3){
